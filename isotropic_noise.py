...
# Directories
parser.add_argument('--data', type=str, default='dafault/data/',
...
parser.add_argument('--opt', type=str, default='sgd',
                    help='')
parser.add_argument('--save_model_per_iter', type=bool, default=False,
                    help='')
parser.add_argument('--noise_alpha', type=float, default=0.02,
                    help='')
...
def normal_noise_adding(model, noise_var):
    grad_correct_factor = args.mbs / float(args.bs)
    for (k, x) in model.named_parameters():
        mu = torch.zeros(x.size()).cuda()
        var = torch.FloatTensor(x.size()).cuda().fill_(noise_var)
        noise = torch.normal(mu, var)
        x.grad.data = torch.add(x.grad.data, noise/grad_correct_factor)

def compute_var(model,optimizer):
    model.train()
    ta = time.time()
    m1 = {}
    m2 = {}
    ...
    for (inputs, targets) in trainloader:
        ...
        loss = criterion(outputs, targets)
        loss.backward()
        ### Compute first on second moment
        for name, variable in model.named_parameters():
            g = variable.grad.data
            if not name in m1.keys():
                m1[name] = g.clone()
                m2[name] = (g * g).clone()
            else:
                m1[name].add_(g)
                m2[name].addcmul_(1., g, g)
        nb_batches += 1
    var = collections.OrderedDict()
    max_variance=0.
    sum_variance = 0.
    for name in m1.keys():
        curm1 = m1[name].abs() / float(nb_batches)
        curm2 = m2[name] / float(nb_batches)
        var[name] = args.mbs*(curm2 - curm1 * curm1)  # + 0.0001
        sum_variance += torch.sum(var[name])
        tmp_max=torch.max(var[name])
        max_variance=tmp_max if tmp_max>max_variance else max_variance
    print('\nComputing variance took {}s'.format(time.time() - ta))
    return var, max_variance, sum_variance

def train(epoch, max_variance):
    global trainloader
    global optimizer
    global args
    global model
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        if train.nb_samples_seen + args.mbs== args.bs:
            normal_noise_adding(model, args.noise_alpha * max_variance)
            optimizer.step()
            compute_angle_et_norm()
            compute_dist()
            optimizer.zero_grad()
            if args.save_model_per_iter is True:
...
if args.opt=='rmsprop':
    ...
    optimizer = torch.optim.SGD(model.parameters(), lr=args.lr,
                                                ...
                                                weight_decay=args.wdecay)
with open(args.save_dir + '/init_model.pt', 'rb') as f:
    checkpoint_i = torch.load(f)
init_model = checkpoint_i['net']
init_optimizer=torch.optim.SGD(init_model.parameters(), lr=args.lr,
                                                momentum=0, \
                                                weight_decay=args.wdecay)
var, max_variance, sum_variance=compute_var(init_model,init_optimizer)
for epoch in range(start_epoch, args.epochs + 1):
    epoch_start_time = time.time()
    loss, train_acc = train(epoch, max_variance)
